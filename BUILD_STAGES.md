...
Status: DONE - except done-when item 5, which is Jayden's: explain types.ts, client.ts, and one full view component without notes. frontend/Dockerfile was written by Claude Code at Jayden's explicit request (a waiver of CLAUDE.md rule 8, recorded in NOTES.md).
...
- The frontend container serves the built app. ✓ verified: `curl http://localhost:3000/` returns the built index.html referencing the hashed `/assets/index-*.js` bundle (same hash as a local `npm run build`), the bundle itself is served with `application/javascript`, and `/positions` returns `index.html` with a 200 (nginx `try_files` fallback for client-side routes). Image is 94 MB: nginx plus `dist/`, no Node.
